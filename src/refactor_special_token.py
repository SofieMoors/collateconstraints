...
--> als je een foutmelding krijgt 'KeyError: 'x' ' dan is het die letter x die mist in replacements en moet je daarvoor
     nog de code zoeken en toevoegen
"""
import os, codecs
...
def make_list(directory, output_path_list, output_path_file):
    ...
    with open(output_path_list, 'w', encoding='utf-8') as output_l: #open een bestand om de lijst op te slaan
        for i in words_with_special_token:
            output_l.write(i.strip() + '\n') # sla de lijst op met op elke regel 1 woord


if __name__ == '__main__':
    input_directory = r'C:\Users\u0144004\OneDrive - KU Leuven\PhD\sofie'
    # dit is de map die alle originele txt's bevat (bvb A.txt)
    output_path_list = r'C:\Users\u0144004\OneDrive - KU Leuven\PhD\words_with_special_token.txt'
    # dit is een bestand waarin alle woorden met een speciaal teken naartoe geschreven worden,
    # op zich heb je dat niet nodig, maar misschien kan dat in de toekomst handig zijn (bvb om mooi te printen)
    output_path_file = r'C:\Users\u0144004\OneDrive - KU Leuven\PhD\correct_unicodes'
    # dit is een nieuwe map waarin een nieuwe versie van elk txt bestand zal worden ingezet.
    # In die bestanden zijn de karakters met streep de juiste code
